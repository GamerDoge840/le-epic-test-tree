    ...
    upgrades: {     
        ...
        3: {    
            fullDisplay() {return `<font size="3"><b>[WO3] Stone Axe</b><font size="2"><br>――――――――――――<br>
                Costs: `+format(tmp[this.layer].upgrades[this.id].costs.Minerals)+` Minerals, `+format(tmp[this.layer].upgrades[this.id].costs.Food)+` Food, and `+format(tmp[this.layer].upgrades[this.id].costs.Wood)+` Wood`},        
            costs: {
                Minerals: 1,
                Food: 10,
                Wood: 4,
            ...
            unlocked() {return (hasUpgrade('Workshop', 2) && hasUpgrade('Research', 11))},
            tooltip(){
